...
import { Context, Effect, Exit, Layer, pipe, Runtime, Scope } from 'effect';
...
export const resource: Effect.Effect<Scope.Scope, never, FileDescriptor> = Effect.acquireRelease(
  pipe(
 ...
 */
export const FileDescriptorLive: Layer.Layer<never, never, FileDescriptor> = Layer.scoped(FileDescriptor, resource);
...
const makeAppRuntime = <R, E, A>(layer: Layer.Layer<R, E, A>) =>
  Effect.gen(function* ($) {
    ...
    const ctx: Context.Context<A> = yield* $(Layer.buildWithScope(scope)(layer));
    const runtime = yield* $(pipe(Effect.runtime<A>(), Effect.provideContext(ctx)));
