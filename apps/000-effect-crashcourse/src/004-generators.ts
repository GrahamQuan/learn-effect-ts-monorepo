...
class CustomRandom extends Context.Tag('CustomRandom')<CustomRandom, { readonly next: () => number }>() {}
...
export const doNotation = pipe(
  ...
  Effect.bind('bar', () => Bar),
  Effect.flatMap(({ random, foo, bar }) =>
    Effect.sync(() => console.log('this is pretty cool!', random.next(), foo.foo, bar.bar)),
  ),
);
