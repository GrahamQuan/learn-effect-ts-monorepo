import { Duration, Effect, Array as EffectArray, Exit, Fiber, pipe } from 'effect';
...
export const example5 = Effect.gen(function* () {
  const identifiers = EffectArray.map(effects, (effect) => Effect.map(effect, (_) => _.id));

  identifiers satisfies readonly Effect.Effect<number>[];

  const sum = Effect.reduceEffect(identifiers, Effect.succeed(0), (acc, a) => acc + a, concurrent);
