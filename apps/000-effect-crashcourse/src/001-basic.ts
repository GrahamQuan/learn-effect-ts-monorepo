...
export const suspend =
  ...
  Effect.suspend(() => (Math.random() > 0.5 ? Effect.succeed(new Date()) : Effect.fail('<.5' as const)));
...
class CustomRandom extends Context.Tag('CustomRandom')<CustomRandom, { readonly next: Effect.Effect<number> }>() {}
...
export const provideServiceExample = serviceExample.pipe(Effect.provideService(CustomRandom, CustomRandomServiceLive));
...
export const liveProgram = pipe(serviceExample, Effect.provide(Layer.succeed(CustomRandom, CustomRandomServiceLive)));
...
export const testProgram = pipe(serviceExample, Effect.provideService(CustomRandom, CustomRandomServiceTest));
