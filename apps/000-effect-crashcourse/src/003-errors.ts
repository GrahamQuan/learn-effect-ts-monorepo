import { Cause, Data, Effect, Equal, identity, Match, Option, pipe } from 'effect';
...
export const isEqual = Equal.equals(FooError({ error: 'foo1' }), FooError({ error: 'foo1' }));

export const isEqualClass = Equal.equals(new FooErrorClass({ error: 'foo1' }), new FooErrorClass({ error: 'foo1' }));

// This is not true, foo1 and foo2 are different!
export const isNotEqual = Equal.equals(FooError({ error: 'foo1' }), FooError({ error: 'foo2' }));
...
example satisfies Effect.Effect<readonly ['success1', 'success2'], FooError | BarError, never>;
 ...
 */
const catchTagSucceed = Effect.catchTag(example, 'FooError', (e) => Effect.succeed(['recover', e.error] as const));
...
catchTagFail satisfies Effect.Effect<readonly ['success1', 'success2'], BarError | BazError, never>;
...
catchTags satisfies Effect.Effect<readonly ['success1', 'success2'] | 'foo' | 'bar', never, never>;
 ...
 */
const catchAll = Effect.catchAll(example, (e) => Effect.succeed(['recover', e._tag] as const));

catchAll satisfies Effect.Effect<
  readonly ['success1', 'success2'] | readonly ['recover', 'FooError' | 'BarError'],
  never,
  never
>;
...
catchSome satisfies Effect.Effect<readonly ['success1', 'success2'] | 'foo', FooError | BarError, never>;
...
fallback satisfies Effect.Effect<readonly ['success1', 'success2'] | 'foo', never, never>;
...
match satisfies Effect.Effect<'FooError' | 'BarError' | 'success1', never, never>;
