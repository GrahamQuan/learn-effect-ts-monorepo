...
export const resource: Effect.Effect<FileDescriptor, never, Scope.Scope> = Effect.acquireRelease(
  pipe(
 ...
 */
export const useFileDescriptorSmarter: useFileDescriptor = Effect.acquireUseRelease(
  Scope.make(),
  (scope) =>
    pipe(
      resource,
...
export const myAcquireUseRelease = <R, E, A, R2, E2, A2, R3, X>(
  ...
  release: (a: A, exit: Exit.Exit<unknown, unknown>) => Effect.Effect<X, never, R3>,
) => pipe(Effect.acquireRelease(acquire, release), Effect.flatMap(use), Effect.scoped);
